...
- 每条新闻新增 `content` 字段（最多 2000 字），并保存为独立的 `.md` 文件
- 文件保存到 `output/content/YYYY-MM-DD/站点名/栏目名/001.md`
- 子进程超时从 120 秒增加到 600 秒

**Phase 3：LLM 子模型总结**
- 启动 `openclaw agent` 子模型（通过 CLI 调用）
- 子模型读取 `output/content/` 目录下的所有 `.md` 文件
- 子模型真正**理解内容**，对每个栏目生成自然语言总结（200-300字），包括：
  - 📝 主要话题和事件概述
  - ⭐ 值得关注的重要新闻
  - 🏷️ 关键主题标签
  - 📊 整体摘要（所有站点的热点和领域覆盖）
- 总结结果写入 `output/analysis-YYYY-MM-DD.json`
- 子模型超时：10 分钟

**飞书文档增强**
- 文档顶部增加「📊 本次爬取分析」整体摘要
- 每个栏目标题下增加：LLM 生成的内容总结 + 主题标签

**用法示例**：
```bash
# 完整流水线（爬取 + 正文下载 + LLM总结 + 飞书文档 + 推送）
node orchestrator.js --content
