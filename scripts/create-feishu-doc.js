#!/usr/bin/env node
// Create a Feishu doc with categorized news data from 3 sources
...
async function createDoc(token, title) {
  const res = await fetch('https://open.feishu.cn/open-apis/docx/v1/documents', {
    ...
    body: JSON.stringify({ title, folder_token: undefined })
  });
  const data = await res.json();
  console.log('Create doc response:', JSON.stringify(data, null, 2));
  return data.data?.document?.document_id;
}

async function getDocBlocks(token, docId) {
  // Get the document's blocks to find the page block
...
async function batchInsertBlocks(token, docId, parentId, blocks, index = -1) {
  ...
  const data = await res.json();
  if (data.code !== 0) {
    console.error('Insert error:', data.msg, JSON.stringify(data).substring(0, 200));
  }
  return data;
}

function makeHeading(text, level = 2) {
  // level: 1-3 maps to heading1-3 in Feishu
  return {
    block_type: level, // 2=heading1, 3=heading2, 4=heading3
    [`heading${level}`]: {
...
function makeHeading2(text) { return makeHeading(text, 2); }  // H1
function makeHeading3(text) { return makeHeading(text, 3); }  // H2
function makeHeading4(text) { return makeHeading(text, 4); }  // H3
...
// News data organized by combined categories
const categories = [
  {
    name: '一、时政要闻',
    sources: [
...
async function main() {
  ...
  // First, add a subtitle/intro
  const introBlocks = [
    makeText('数据来源：新浪新闻、观察者网、钛媒体 | 按栏目分类整理 | 共10个栏目 | 爬取时间：2026-03-17 13:00'),
    makeText(''),
  ];
  await batchInsertBlocks(token, docId, docId, introBlocks);
  ...
  for (const cat of categories) {
    ...
    const headingBlocks = [makeHeading3(cat.name)];
    await batchInsertBlocks(token, docId, docId, headingBlocks);

    for (const source of cat.sources) {
      // Add source sub-heading
      const sourceBlocks = [makeHeading4(`【${source.site}】`)];
      await batchInsertBlocks(token, docId, docId, sourceBlocks);

      // Add items in batches of 10 (API limit)
      const bullets = source.items.map(([title, url], i) => 
        makeBullet(`${i + 1}. ${title} — ${url}`)
      );

      for (let i = 0; i < bullets.length; i += 10) {
        const batch = bullets.slice(i, i + 10);
        await batchInsertBlocks(token, docId, docId, batch);
      }
    ...
    // Add separator
    await batchInsertBlocks(token, docId, docId, [makeText('')]);
  }

  console.log(`\nDone! Document URL: https://szzhgl.feishu.cn/docx/${docId}`);
}
